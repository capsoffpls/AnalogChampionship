...
execute as @e[type=item_display,tag=bonus,tag=!collected] at @s run execute as @e[type=horse,dy=1,limit=1] on passengers if score @s AC_hknVelocity matches 350 run function airconditioner:games/hkn/bonus-activate

execute as @e[type=item_display,tag=bonus,tag=collected] run scoreboard players add @s AC_hknBonusBackend 1
execute as @e[type=item_display,tag=bonus,tag=collected] if score @s AC_hknBonusBackend matches 1 run data modify entity @s transformation.scale set value [0.0f,0.0f,0.0f]

execute as @e[type=item_display,tag=bonus,tag=collected] if score @s AC_hknBonusBackend matches 200.. run data modify entity @s transformation.scale set value [1.0f,1.0f,1.0f]
