...
execute if entity @s[tag=cstArsonist] run function airconditioner:games/cst/passives/arsonist
...
execute if entity @s run scoreboard players add @a[scores={AC_killmessage=1..}] AC_sillyPointsHeld 5
...
function airconditioner:dev/low/cst_append_death
execute as @a[scores={AC_killmessage=1..}] run function airconditioner:dev/low/cst_append_kill