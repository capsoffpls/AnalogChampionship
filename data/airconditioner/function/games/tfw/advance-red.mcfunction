...
execute as @a[team=ac_tfwRed] at @s run playsound minecraft:ac.jingles.success record @s ~ ~ ~
scoreboard players add @a[team=ac_tfwRed,scores={AC_killmessage=1..}] AC_sillyPointsHeld 5
tellraw @a[team=ac_tfwRed,scores={AC_killmessage=1..}] [{"text":"[TFW] ","bold":true,"color":"dark_green"},{"text":"+5≡","bold":false,"color":"gold"}]
scoreboard players remove score AC_tfwCheck 1