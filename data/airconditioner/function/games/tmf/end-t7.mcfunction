...
scoreboard players add @a[tag=InGame,team=ac_tmf7] AC_sillyPointsHeld 30
execute if score lang AC_lang matches 0 run tellraw @a[tag=InGame,team=ac_tmf7] [{"text":"[TMF] ","bold":true,"color":"dark_green"},{"text":"+30≡ (Zwycięstwo)","color":"gold","bold":false}]
execute if score lang AC_lang matches 1 run tellraw @a[tag=InGame,team=ac_tmf7] [{"text":"[TMF] ","bold":true,"color":"dark_green"},{"text":"+30≡ (Victory)","color":"gold","bold":false}]
