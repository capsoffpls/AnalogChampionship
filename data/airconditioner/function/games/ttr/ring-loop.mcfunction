...
function airconditioner:games/ttr/ring-draw
tag @a[tag=showCircle] remove showCircle


################### detect player
execute positioned 1960 -64 ~-2 as @a[gamemode=adventure,tag=InGame,dx=80,dz=3,dy=384] if score @s AC_ttrRing < @n[tag=this] AC_ttrScores run function airconditioner:games/ttr/pass-check-player
