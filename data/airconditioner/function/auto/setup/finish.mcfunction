...
execute unless score mode AC_autoControl matches 3 run tellraw @s [{text:"[Powrót]",bold:true,color:gray,click_event:{action:"run_command",command:"function airconditioner:menu/start"}}]
execute if score mode AC_autoControl matches 3 run tellraw @s [{text:"[Powrót] ",bold:true,color:gray,click_event:{action:"run_command",command:"function airconditioner:menu/start"}},{text:"[Ustaw pulę trybów]",bold:true,color:gold,click_event:{action:"run_command",command:"function airconditioner:menu/draw-menu {\"return\":\"function airconditioner:menu/start\"}"}}]
