...
#execute if entity @a[tag=spinning,name=MikoGames20] run tellraw @a[tag=dev] [{"text":"[AC] ","bold":true,"color":"white"},{"text":"Miko wlasnie trafil ","color":"gray","bold":false},{"score":{"name":"temp_win","objective":"AC_misc"},"bold":true, "color":"white"},{"text":" AP","color":"white","bold":true}]
...
scoreboard players operation @a[tag=spinning] AC_sillyPoints += temp_win AC_misc 
